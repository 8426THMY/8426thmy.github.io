...
\noindent \textcolor{red}{Exposition. Introduce this technology.}
\newline
...
\noindent \textcolor{red}{Maybe clean up the notation in this section. It might be good to use bars for scalar complex conjugation rather than stars. Also, I'm not sure which way to do the labels for the constants. The current way means you don't have to use the fact that $H$ is Abelian to say that $\lambda$ and $\rho$ are representations (as opposed to anti-representations). However, it also conflicts with the normal way of ascribing matrices to these morphisms, so the multiplication appears backwards when we do the algebra object isomorphisms.}
\newline
...
\sum_{h' \in H}{\abs{\psi(h(h')^{-1}, h')}^2} + \tr(s(h)s(h)^{*}) = \dim(A), \\
...
\end{align*}
\noindent for all $h, h' \in H$ and $i \in \{1, 2, \dots, a\}$. \textcolor{red}{If we write the unprimed functions in terms of the primed ones and plug them back into our equations, do $f_H$ and $f_X$ vanish?}
\end{proposition}
\leavevmode\newline
\begin{proof}
Let $(A, m_A, u_A)$ and $(A, m_A', u_A')$ denote the algebra objects associated to the solutions, and consider any (unitary) isomorphism of algebra objects \textcolor{red}{(we still need to write down what this means!)}
\begin{align*}
\begin{split}
...
\end{split}
\end{align*}
\noindent As before, the terms in these sums are linearly independent. Thus, equating $m_A' f A(f) = f m_A'$, we obtain the desired results.
\end{proof}
\leavevmode\newline
...
\end{alignat*}
\noindent To assume $\psi(h, h^{-1}) = 1 = \psi(h^{-1}, h)$, can we just use \hyperref[prop:near-group_algebra_isomorphism]{Proposition \ref*{prop:near-group_algebra_isomorphism}}? If two algebra objects are isomorphic, then so are their module categories. Any standard $Q$-system is going to be isomorphic to an algebra object with a normalized 2-cocycle, so we can maybe just write down the $Q$-system equations in terms of the normalized data.}
\newline

\noindent Unfortunately, in the general case, \hyperref[eq:near-group_algebra_ugly_start]{Equations \ref*{eq:near-group_algebra_ugly_start} to \ref*{eq:near-group_algebra_ugly_end}} are difficult to reduce. Fortunately, things become much simpler when we assume $m = 1$. Here, the morphisms $T_x$ are indexed by elements of $G$, and we have explicit equations for $X(S_h)$, $X(T_x)$ and $X(U(h))$ in as polynomials in $S_h$ and $T_x$.
\newline

\noindent \textcolor{red}{Verify the equations for $m = 1$. I may have deleted some of the inner products by falsely assuming they're 1 on elements of $H$.}
\newline

\begin{proposition}\label{prop:near-group_algebra_equations}
Let $\mathcat{C}$ be a unitary near-group fusion category of type $(G, m\abs{G})$ for some finite Abelian group $G$ and $m \in \mathbb{Z}_{>0}$, and let $V = \mathbb{C}^a$ for $a \in \mathbb{Z}_{\geq 0}$ with basis $(e_1, e_2, \dots, e_a)$. Then the object $A = H \oplus X^{\oplus a}$ admits the structure of a standard $Q$-system if there exists a 2-cocycle $\psi : H^2 \to \mathbb{C}^{\times}$, projective representations $\lambda, \rho : H \to \GL(V)$ and functions $s : H \to \End[\mathbb{C}]{V}$, $t : G \to \Hom[\mathbb{C}]{V, \End[\mathbb{C}]{V}}$ satisfying
\begin{gather*}
\sum_{h'' \in H}{\abs{\psi(h(h'')^{-1}, h'')}^2} + \tr(s(h)s(h)^{*}) = \dim(A), \\
\sum_{h'' \in H}{(\lambda(h'')\lambda(h'')^{*})_{i,j}} + \sum_{h'' \in H}{(\rho(h'')\rho(h'')^{*})_{i,j}} + \sum_{x=1}^{m\abs{G}}{\tr([t(x)](e_i)[t(x)](e_j)^{*})} = \delta_{i,j}\dim(A),
\end{gather*}
...
\sum_{x=1}^{m\abs{G}}{s_{h,i,r}t_{x,r,j,k}S_h^{*}T_x^{*}} &= \sum_{x=1}^{m\abs{G}}{s_{h,r,k}t_{x,r,i,j}S_h^{*}X(T_x^{*})},
\end{align*}
\begin{equation*}
\begin{alignedat}{2}
&\sum_{h'' \in H}{\lambda_{h'',i,j}s_{h'',k,l}S_{h''}^{*}} &&+ \sum_{x,y=1}^{m\abs{G}}{t_{x,i,j,r}t_{y,r,k,l}T_x^{*}T_y^{*}} = \\
&\sum_{h'' \in H}{\rho_{h'',i,l}s_{h'',j,k}U(h'')^{*}X(S_{h''}^{*})} &&+ \sum_{x,y=1}^{m\abs{G}}{t_{x,i,r,l}t_{y,r,j,k}T_x^{*}X(T_y^{*})},
\end{alignedat}
\end{equation*}
...
[t(xh)](e_i^T\rho(h)) &= \inprod{h, x}^{*}a(h)^{*}\rho(h)^T [t(x)](e_i).
\end{align*}
\begin{gather*}
\sum_{r=1}^{a}{s_{h,i,r}t_{x,r,j,k}} = \frac{c}{\sqrt{\abs{G}}}a(h)\inprod{x, h}^{*}\sum_{z \in G}{\sum_{r=1}^{a}{\inprod{x, z}^{*}s_{h,r,k}t_{z,r,i,j}}}, \\
d^{1/2}\delta_{x \in H}\lambda_{x,i,j}s_{x,k,l} = d^{-1/2}\sum_{z \in H}{\inprod{x, z}^{*}\rho_{z,i,l}s_{z,j,k}} + \frac{c^{*}}{\sqrt{\abs{G}}}\sum_{z,w \in G}{\sum_{r=1}^{a}{\inprod{w, z}^{*}\inprod{x, w}t_{w,i,r,l}t_{z,r,j,k}}}, \\
\sum_{r=1}^{a}{t_{x,r,i,j}t_{y,r,k,l}} = d^{-1/2}\delta_{xy \in H}a(x^{-1})^{*}\rho_{(xy)^{-1},i,l}s_{(xy)^{-1},j,k} + \sum_{z \in G}{\sum_{r=1}^{a}{a(x^{-1})^{*}b(zx^{-1})^{*}\inprod{xy, z}^{*}t_{xy,i,r,l}t_{z,r,j,k}}},
\end{gather*}
...
\begin{corollary}
Let $\mathcat{C}$ be a unitary near-group fusion category of type $(G, \abs{G})$ for some finite Abelian group $G$, and consider any subgroup $H \leq G$. Then $A = H \oplus X$ admits the structure of a standard $Q$-system if $\inprod{h, h'} = 1$ for all $h, h' \in H$, and there exists a 2-cocycle $\psi : H^2 \to \mathbb{C}^{\times}$, projective representations $\lambda, \rho : H \to \mathbb{C}^{\times}$ and a function $t : G \to \mathbb{C}$ satisfying
\begin{gather*}
\sum_{h'' \in H}{\abs{\psi(h(h'')^{-1}, h'')}^2} + \abs{s(h)}^2 = \dim(A), \\
...
\rho(h)\rho(h') &= \psi(h', h)\rho(hh'), \\
s(hh')\lambda(h) &= \psi(h, h')s(h'), \\
\rho(h)s(hh') &= \psi(h', h)s(h'), \\
\lambda(h)s(h') &= s(h')\rho(h),
\end{align*}
\begin{align*}
t(x) &= \inprod{h, x}^{*}t(x), \\
\lambda(h)t(x) &= t(x)\rho(h), \\
t(xh)\rho(h) &= \inprod{h, x}^{*}a(h)^{*}\rho(h)t(x),
\end{align*}
\begin{gather*}
t(x) = \frac{c}{\sqrt{\abs{G}}}a(h)\inprod{x, h}^{*}\sum_{z \in G}{\inprod{x, z}^{*}t(z)}, \\
d^{1/2}\delta_{x \in H}\lambda(x)s(x) = d^{-1/2}\sum_{z \in H}{\inprod{x, z}^{*}\rho(z)s(z)} + \frac{c^{*}}{\sqrt{\abs{G}}}\sum_{z,w \in G}{\inprod{w, z}^{*}\inprod{x, w}t(w)t(z)}, \\
t(x)t(y) = d^{-1/2}\delta_{xy \in H}a(x^{-1})^{*}\rho((xy)^{-1})s((xy)^{-1}) + \sum_{z \in G}{a(x^{-1})^{*}b(zx^{-1})^{*}\inprod{xy, z}^{*}t(xy)t(z)},
\end{gather*}
\textcolor{red}{Finish this, and double check the last three equations.}
\end{corollary}
\leavevmode
\newpage
